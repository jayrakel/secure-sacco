...
const exportSavingsCSV = (rows: StatementTransactionResponse[]) => {
    ...
    a.href     = url; a.download = `MySavingsStatement_${today()}.csv`; a.click();
...
const MemberPersonalReportsPage: React.FC = () => {
    ...
    const fetchSavings = useCallback(async () => {
        setSavingsLoading(true); setSavingsError('');
        try {
            const data = await savingsApi.getMyStatement(savingsFrom, savingsTo);
            setSavingsTxns(data);
            setSavingsRan(true);
        } catch (e: any) {
            setSavingsError(e?.response?.data?.message ?? 'Failed to load statement. Please try again.');
        } finally {
    ...
    const totalDeposits    = useMemo(() => savingsTxns.filter(t => t.type === 'DEPOSIT').reduce((s, t) => s + t.amount, 0), [savingsTxns]);
    const totalWithdrawals = useMemo(() => savingsTxns.filter(t => t.type === 'WITHDRAWAL').reduce((s, t) => s + t.amount, 0), [savingsTxns]);
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex flex-wrap items-end gap-3">
                        <div className="flex flex-col gap-1">
                            ...
                            <input type="date" value={savingsFrom} max={savingsTo}
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <input type="date" value={savingsTo} min={savingsFrom} max={today()}
                                   onChange={e => setSavingsTo(e.target.value)}
                                   className="text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
                            />
                        </div>
                        <button onClick={fetchSavings} disabled={savingsLoading}
                        ...
                        {savingsRan && savingsTxns.length > 0 && (
                            <button onClick={() => exportSavingsCSV(savingsTxns)}
                    ...
                    {savingsError && (
                        <div className="p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl text-sm">{savingsError}</div>
                    )}

                    {/* Summary cards (after run) */}
                    {savingsRan && !savingsLoading && (
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                                ...
                                <p className="text-xs text-slate-400 mt-1">{savingsTxns.filter(t => t.type === 'DEPOSIT').length} transactions</p>
                            </div>
                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                                ...
                                <p className="text-xs text-slate-400 mt-1">{savingsTxns.filter(t => t.type === 'WITHDRAWAL').length} transactions</p>
                            </div>
                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                    ...
                    {savingsLoading ? (
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                            <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between">
                                ...
                                <p className="text-xs text-slate-400">{savingsTxns.length} record{savingsTxns.length !== 1 ? 's' : ''}</p>
                            </div>
                                    ...
                                    <table className="min-w-full divide-y divide-slate-100">
                                        ...
                                        <tr>
                                            {['Date', 'Type', 'Reference', 'Channel', 'Amount (KES)', 'Balance (KES)', 'Status'].map(h => (
                                                <th key={h} className={`px-5 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider ${h.includes('Amount') || h.includes('Balance') ? 'text-right' : 'text-left'}`}>{h}</th>
                                            ))}
                                        </tr>
                                        </thead>
                                            ...
                                            return (
                                                <tr key={tx.transactionId} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="px-5 py-3.5 whitespace-nowrap">
                                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${isDeposit ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                                                                {isDeposit
                                                    ...
                                                    <td className={`px-5 py-3.5 whitespace-nowrap text-right text-sm font-bold ${isDeposit ? 'text-emerald-600' : 'text-amber-600'}`}>
                                                    ...
                                                    <td className="px-5 py-3.5 whitespace-nowrap">
                                                            <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                                                                tx.status === 'POSTED' ? 'bg-emerald-50 text-emerald-700' :
                                                                    tx.status === 'PENDING' ? 'bg-sky-50 text-sky-700' :
                        ...
                        <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-2">
                            ...
                            <p className="text-sm font-medium text-slate-500">Select a date range and click <strong className="text-slate-600">Load Statement</strong></p>
                        </div>
                    )}
                </div>
            )}
            ...
            {activeTab === 'loans' && (
                <div className="space-y-3">
                    {loansLoading ? (
                        ...
                        const cfg = LOAN_STATUS[loan.status] ?? { badge: 'bg-slate-100 text-slate-600', dot: 'bg-slate-400', label: loan.status };
                                        ...
                                        {loan.status === 'ACTIVE' && !summary ? (
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                                {[
                                                    ...
                                                    { label: 'Application Fee',  value: `KES ${fmt(loan.applicationFee)}`, sub: loan.applicationFeePaid ? '✓ Paid' : '✗ Unpaid' },
                                                    { label: 'Status',           value: cfg.label },
                                                    ...(summary ? [
                                                        { label: 'Outstanding',  value: `KES ${fmt(summary.totalOutstanding)}`, highlight: true },
                                                        { label: 'Arrears',      value: summary.totalArrears > 0 ? `KES ${fmt(summary.totalArrears)}` : 'None', danger: summary.totalArrears > 0 },
                                                        { label: 'Next Due',     value: summary.nextDueDate ? format(new Date(summary.nextDueDate), 'dd MMM yyyy') : '—' },
                                                        { label: 'Due Amount',   value: summary.nextDueAmount > 0 ? `KES ${fmt(summary.nextDueAmount)}` : '—' },
                                                        { label: 'Prepayment Cr',value: `KES ${fmt(summary.prepaymentCredit)}` },
                                                    ] : []),
                                                ].map((item, i) => (
                                                    <div key={i} className="bg-white rounded-xl p-3 border border-slate-100">
                                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{item.label}</p>
                                                        <p className={`text-sm font-bold mt-0.5 ${
                                                            (item as any).danger ? 'text-rose-600' :
                                                                (item as any).highlight ? 'text-violet-700' :
                                                                    'text-slate-800'
                                                        }`}>{item.value}</p>
                                                        {(item as any).sub && (
                                                            <p className="text-[10px] text-slate-400 mt-0.5">{(item as any).sub}</p>
                                                        )}
                                                    </div>
                                                ))}
                                                ...
                                                {loan.guarantors?.length > 0 && (
                                                    <div className="col-span-2 sm:col-span-3 bg-white rounded-xl p-3 border border-slate-100">
                                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Guarantors ({loan.guarantors.length})</p>
                                                        <div className="flex flex-wrap gap-2">
                                                            {loan.guarantors.map(g => (
                                                                <span key={g.id} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-violet-50 text-violet-700 rounded-full text-xs font-semibold">
                    ...
                    {!penaltiesLoading && penalties.length > 0 && (
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                                ...
                                <p className={`text-2xl font-bold mt-2 ${totalPenaltiesOutstanding > 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                                    KES {fmt(totalPenaltiesOutstanding)}
                                </p>
                                <p className="text-xs text-slate-400 mt-1">{penalties.filter(p => p.status === 'OPEN' || p.status === 'PARTIAL').length} open</p>
                            </div>
                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                                ...
                                <p className="text-xs text-slate-400 mt-1">{penalties.length} total record{penalties.length !== 1 ? 's' : ''}</p>
                            </div>
                            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                                ...
                                <table className="min-w-full divide-y divide-slate-100">
                                    ...
                                    <tr>
                                        {['Rule', 'Date Raised', 'Original Amount', 'Amount Paid', 'Waived', 'Outstanding', 'Status'].map(h => (
                                            <th key={h} className={`px-5 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider ${['Original Amount','Amount Paid','Waived','Outstanding'].includes(h) ? 'text-right' : 'text-left'}`}>{h}</th>
                                        ))}
                                    </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-50 bg-white">
                                    {penalties.map(p => (
                                        <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="px-5 py-3.5 whitespace-nowrap">
                                                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${PENALTY_STATUS[p.status] ?? 'bg-slate-100 text-slate-600'}`}>
                                                        {p.status}
                                                    </span>
                                            </td>
                                        </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td className="px-5 py-3 text-right text-sm font-bold text-slate-700">{fmt(penalties.reduce((s, p) => s + p.originalAmount, 0))}</td>
                                        <td className="px-5 py-3 text-right text-sm font-bold text-emerald-600">{fmt(penalties.reduce((s, p) => s + p.principalPaid + p.interestPaid, 0))}</td>
                                        <td className="px-5 py-3 text-right text-sm font-bold text-slate-500">{fmt(penalties.reduce((s, p) => s + p.amountWaived, 0))}</td>
                                        <td className="px-5 py-3 text-right text-sm font-bold text-rose-600">{fmt(totalPenaltiesOutstanding)}</td>
                                        <td />
                                    </tr>
                                    </tfoot>
                                </table>
