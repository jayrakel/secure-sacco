...
export default function MeetingsManagementPage() {
    ...
    const handleQrClick = (e: React.MouseEvent, m: Meeting) => { e.stopPropagation(); setQrMeeting(m); };
    ...
    return (
        <>
            {qrMeeting && (
                <MeetingQrModal
                    meeting={{ id: qrMeeting.id, title: qrMeeting.title, startAt: qrMeeting.startAt, qrToken: qrMeeting.qrToken ?? '' }}
                    onClose={() => setQrMeeting(null)}
                    onTokenRegenerated={(newToken) => {
                        setMeetings(prev => prev.map(m => m.id === qrMeeting.id ? { ...m, qrToken: newToken } : m));
                        setQrMeeting(prev => prev ? { ...prev, qrToken: newToken } : null);
                    }}
                />
            )}
