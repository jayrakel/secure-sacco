...
const inputCls = "w-full px-3 py-2.5 border border-slate-300 rounded-xl text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white placeholder-slate-300 transition";
...
const SaccoSettingsPage: React.FC = () => {
    ...
    const [enabledModules, setEnabledModules] = useState<Record<string, boolean>>({
        members: true, loans: false, savings: false, reports: false,
    });
    ...
    useEffect(() => {
        ...
        const id = setTimeout(async () => {
            try { const d = await settingsApi.generatePrefix(saccoName); setPrefix(d.prefix); }
            catch {}
        }, 500);
    ...
    const handleCoreSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setIsSavingCore(true); setError(''); setSuccess('');
        const payload = { saccoName, prefix: prefix.toUpperCase(), padLength, registrationFee };
        try {
    ...
    const handleFlagsSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setIsSavingFlags(true); setError(''); setSuccess('');
        try {
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 pb-12">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                        {settings?.initialized ? 'Update your SACCO name, member number format and fee.' : 'Set up your SACCO identity to get started.'}
                    </p>
                </div>
                ...
                <form onSubmit={handleCoreSubmit} className="p-6 space-y-5">
                    <Field label="Official SACCO Name">
                        <input
                            type="text" required value={saccoName}
                        ...
                        >
                            <div className="relative">
                                <input
                                    ...
                                    value={prefix}
                                    onChange={e => { setPrefix(e.target.value); setPrefixManuallyEdited(true); }}
                                    className={`${inputCls} uppercase`}
                                    placeholder="UMJ"
                                />
                                {!prefixManuallyEdited && prefix && (
                                    <span className="absolute right-3 top-2.5 text-[10px] text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded font-semibold">Auto</span>
                                )}
                            </div>
                        </Field>
                        <Field
                            label="Pad Length"
                            hint="Number of zero-padded digits after the prefix."
                        >
                            <input
                                type="number" required min={1} value={padLength}
                    ...
                    >
                        <input
                            type="number" required min={0} value={registrationFee}
            ...
            {/* ── Modules card ── */}
            <div className={`bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden transition-opacity ${!settings?.initialized ? 'opacity-50 pointer-events-none' : ''}`}>
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50">
                    <h2 className="text-sm font-bold text-slate-800">System Modules</h2>
                    <p className="text-xs text-slate-500 mt-0.5">Enable or disable core system features. Changes reflect in the sidebar immediately.</p>
                </div>
                    ...
                    <div className="space-y-3">
                        {Object.entries(MODULE_LABELS).map(([key, { label, desc }]) => (
                            <div key={key} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${
                                enabledModules[key] ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-100 bg-slate-50/50'
                            }`}>
                                <div>
                                ...
                                <button
                                    type="button"
                                    onClick={() => setEnabledModules(prev => ({ ...prev, [key]: !prev[key] }))}
