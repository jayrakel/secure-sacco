...
export default function UserSessionsModal({ userId, userName, onClose }: Props) {
    ...
    const fetchSessions = useCallback(async () => {
        ...
        try {
            ...
            setSessions(
                data.sort(
                    (a, b) =>
                        new Date(b.lastAccessedTime).getTime() -
                        new Date(a.lastAccessedTime).getTime()
                )
            );
        } catch (error: unknown) {
    ...
    const handleRevokeSingle = async (sessionId: string) => {
        if (!window.confirm('Terminate this session? The user will be logged out on that device.')) {
            return;
        }
        ...
        try {
            ...
            alert(getApiErrorMessage(error, 'Failed to terminate session.'));
        }
    };

    const handleRevokeAll = async () => {
        if (!window.confirm('Terminate ALL sessions for this user? They will be immediately locked out on all devices.')) {
            return;
        }
        ...
        try {
            ...
            alert(getApiErrorMessage(error, 'Failed to terminate all sessions.'));
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">
                            Viewing devices currently logged in as{' '}
                ...
                <div className="p-6 min-h-75 max-h-[60vh] overflow-y-auto">
                    {isLoading ? (
                        ...
                        <div className="space-y-3">
                            {sessions.map((session, idx) => (
                                <div
                                    ...
                                    className="flex justify-between items-center p-4 rounded-xl border border-slate-200 bg-white hover:border-purple-300 transition group"
                                >
                ...
                <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-between items-center">
                    <p className="text-xs text-slate-500">
                        Note: Terminating a session takes effect immediately on the next API call.
                    </p>
                    <HasPermission permission="SESSION_REVOKE">
                        <button
