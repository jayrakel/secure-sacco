...
public class UserActivationService {
    ...
    private void generateEmailToken(User user) {
        ...
        // OTP is stored in DB; both token and OTP are sent together so the member can complete activation in one step.
        // The OTP for this user was just generated by generateOtp() before this call — retrieve it from the DB.
        String latestOtp = tokenRepository
                .findFirstByUserIdAndTokenTypeAndIsUsedFalseOrderByCreatedAtDesc(user.getId(), VerificationTokenType.PHONE_ACTIVATION)
                .map(VerificationToken::getToken)
                .orElse("------");
        String activationUrl = frontendUrl + "/activate?token=" + uuidToken;
        emailNotificationService.sendActivationEmail(user.getEmail(), activationUrl, latestOtp);
        log.info("📧 Activation email dispatched to {}", user.getEmail());
    }
