...
public class DashboardService {
    ...
    public StaffDashboardDTO getStaffDashboardMetrics() {
        ...
        String sql = """
            SELECT
                ...
                COALESCE((SELECT SUM(outstanding_amount) FROM penalties WHERE status = 'OPEN'), 0) AS outstanding_penalties,

                COALESCE((SELECT SUM(amount) FROM payments
                    WHERE status = 'COMPLETED'
                      AND CAST(created_at AS DATE) = CURRENT_DATE), 0)                     AS todays_collections,

                (SELECT COUNT(*) FROM meetings WHERE status = 'SCHEDULED' AND start_at >= CURRENT_TIMESTAMP) AS upcoming_meetings,
                (SELECT COUNT(*) FROM meetings
                    WHERE EXTRACT(MONTH FROM start_at) = EXTRACT(MONTH FROM CURRENT_DATE)
    ...
    public MemberDashboardDTO getMemberDashboardMetrics(String identifier) {
        log.info("📊 Executing member dashboard DB query for: {} (Cache Miss)", identifier);
        ...
        String sql = """
            SELECT
                ...
                -- Savings
                COALESCE((SELECT vsb.total_savings_balance
                          FROM v_member_savings_balance vsb
                          WHERE vsb.member_id = m.id), 0)              AS savings_balance,
                COALESCE((SELECT SUM(t.amount)
                          FROM savings_transactions t
                          JOIN savings_accounts sa ON t.savings_account_id = sa.id
                          WHERE sa.member_id = m.id
                            AND t.type = 'DEPOSIT' AND t.status = 'POSTED'), 0) AS total_deposited,
                COALESCE((SELECT SUM(t.amount)
                          FROM savings_transactions t
                          JOIN savings_accounts sa ON t.savings_account_id = sa.id
                          WHERE sa.member_id = m.id
                            AND t.type = 'WITHDRAWAL' AND t.status = 'POSTED'), 0) AS total_withdrawn,

                -- Loans
                (SELECT COUNT(*) FROM loan_applications
                 WHERE member_id = m.id
                   AND status IN ('ACTIVE','IN_GRACE','DEFAULTED'))    AS active_loans,
                COALESCE((SELECT SUM(lsi.total_due - lsi.principal_paid - lsi.interest_paid)
                          FROM loan_schedule_items lsi
                          JOIN loan_applications la ON lsi.loan_application_id = la.id
                          WHERE la.member_id = m.id
                            AND la.status IN ('ACTIVE','IN_GRACE','DEFAULTED')), 0) AS loan_outstanding,

                -- Next installment
                (SELECT lsi.total_due - lsi.principal_paid - lsi.interest_paid
                 FROM loan_schedule_items lsi
                 JOIN loan_applications la ON lsi.loan_application_id = la.id
                 WHERE la.member_id = m.id AND lsi.status != 'PAID'
                   AND la.status IN ('ACTIVE','IN_GRACE','DEFAULTED')
                 ORDER BY lsi.due_date LIMIT 1)                        AS next_installment_amount,
                (SELECT lsi.due_date
                 FROM loan_schedule_items lsi
                 JOIN loan_applications la ON lsi.loan_application_id = la.id
                 WHERE la.member_id = m.id AND lsi.status != 'PAID'
                   AND la.status IN ('ACTIVE','IN_GRACE','DEFAULTED')
                 ORDER BY lsi.due_date LIMIT 1)                        AS next_installment_due_date,

                -- Penalties
                (SELECT COUNT(*)  FROM penalties WHERE member_id = m.id AND status = 'OPEN') AS open_penalties_count,
                COALESCE((SELECT SUM(outstanding_amount) FROM penalties
                          WHERE member_id = m.id AND status = 'OPEN'), 0)                    AS open_penalties_amount,

                -- Upcoming meeting (the very next one)
                (SELECT COUNT(*) FROM meetings
                 WHERE status = 'SCHEDULED' AND start_at >= CURRENT_TIMESTAMP)               AS upcoming_meetings,
                (SELECT CAST(id AS TEXT)    FROM meetings
                 WHERE status = 'SCHEDULED' AND start_at >= CURRENT_TIMESTAMP
                 ORDER BY start_at LIMIT 1)                                                  AS upcoming_meeting_id,
                (SELECT title               FROM meetings
                 WHERE status = 'SCHEDULED' AND start_at >= CURRENT_TIMESTAMP
                 ORDER BY start_at LIMIT 1)                                                  AS upcoming_meeting_title,
                (SELECT CAST(start_at AS TEXT) FROM meetings
                 WHERE status = 'SCHEDULED' AND start_at >= CURRENT_TIMESTAMP
                 ORDER BY start_at LIMIT 1)                                                  AS upcoming_meeting_start_at,
                (SELECT status              FROM meetings
                 WHERE status = 'SCHEDULED' AND start_at >= CURRENT_TIMESTAMP
                 ORDER BY start_at LIMIT 1)                                                  AS upcoming_meeting_status,

                -- Attendance rate (% of meetings where member was PRESENT)
                COALESCE((SELECT CASE WHEN COUNT(*) = 0 THEN 0
                           ELSE CAST(SUM(CASE WHEN ma.status = 'PRESENT' THEN 1 ELSE 0 END) * 100.0 / COUNT(*) AS INT)
                           END
                          FROM meeting_attendance ma WHERE ma.member_id = m.id), 0)          AS attendance_rate
        ...
        return jdbcTemplate.queryForObject(sql, (rs, rowNum) -> {
            ...
            dto.setActiveLoans(rs.getInt("active_loans"));
            dto.setLoanOutstanding(rs.getBigDecimal("loan_outstanding"));
            ...
            if (dueDate != null) dto.setNextInstallmentDueDate(dueDate.toLocalDate().toString());

            dto.setOpenPenaltiesCount(rs.getInt("open_penalties_count"));
            dto.setOpenPenaltiesAmount(rs.getBigDecimal("open_penalties_amount"));

            dto.setUpcomingMeetings(rs.getInt("upcoming_meetings"));
            dto.setUpcomingMeetingId(rs.getString("upcoming_meeting_id"));
            dto.setUpcomingMeetingTitle(rs.getString("upcoming_meeting_title"));
            dto.setUpcomingMeetingStartAt(rs.getString("upcoming_meeting_start_at"));
            dto.setUpcomingMeetingStatus(rs.getString("upcoming_meeting_status"));
