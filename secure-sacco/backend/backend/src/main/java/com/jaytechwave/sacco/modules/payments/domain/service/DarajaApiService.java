...
public class DarajaApiService {
        ...
        public StkPushSyncResponse initiateStkPush(String phoneNumber, String amount, String accountReference,
                        String transactionDesc) {
                String token = getAccessToken();
                String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));
                String password = Base64.getEncoder().encodeToString(
                                (darajaProperties.getShortcode() + darajaProperties.getPasskey() + timestamp)
                                                .getBytes(StandardCharsets.UTF_8));

                String callbackUrl = darajaProperties.getCallbackBaseUrl()
                                + darajaProperties.getEndpoints().getMpesaExpressCallback();
